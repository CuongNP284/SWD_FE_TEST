...
const LoginPage: React.FC = () => {
  ...
  const overlayBg: string = "bg-gradient-to-r from-sky-950 via-sky-800 to-sky-950";

  return (
    <div className="h-4/5 w-3/4 bg-white-100 relative overflow-hidden no-scrollbar rounded-lg">
      <div
        id="signin"
        className={`bg-white-100 absolute top-0 left-0 h-full w-1/2 flex justify-center items-center transition-all duration-700 ease-in-out z-20 ${isAnimated ? "translate-x-full opacity-0" : ""
          }`}
      >
        <SigninForm />
      </div>
